...
__device__ bool checkWord(char* str, int idx, char* key, int keylen) {
...
__global__ void wordMatchOnString(char* str, int len, char* key, int keylen, int *count) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id < len - keylen + 1) {
        if (checkWord(str, id, key, keylen)) {
            atomicAdd(count, 1);
        }
    }
}
...
int main() {
    ...
    wordMatchOnString<<<ceil((float)len/256), 256>>>(d_str, len, d_key, keylen, d_count);
