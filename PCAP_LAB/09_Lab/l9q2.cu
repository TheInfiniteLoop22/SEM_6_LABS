...
__global__ void transformMatrix(int *A, int M, int N) {
    ...
    if (row < M) {
        for (int col=0; col<N; col++) {
            A[row * N + col] = (int)roundf(pow((float)A[row * N + col], (float)(row + 1)));
        }
    }
}
...
int main() {
    ...
    // Updated grid and block dimensions to handle larger M
    int threadsPerBlock = 256;
    int blocksPerGrid = (M + threadsPerBlock - 1) / threadsPerBlock;
    
    transformMatrix<<<blocksPerGrid, threadsPerBlock>>>(d_A, M, N);
